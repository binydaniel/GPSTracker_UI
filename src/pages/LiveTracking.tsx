...
//
// export default function LiveTracking() {
//   // Initialize with an empty array or loading state instead of bad mock IDs
//   const [vehicles, setVehicles] = React.useState<Vehicle[]>([]);
//   const [apiLocations, setApiLocations] = React.useState<ApiLocationData[]>([]);
//   const [selectedVehicle, setSelectedVehicle] = React.useState<Vehicle | null>(null);
//   const [searchQuery, setSearchQuery] = React.useState('');
//
//   const [mapConfig, setMapConfig] = React.useState<{ center: [number, number]; zoom: number }>({
//     center: [9.01377, 38.7101783], // Default center: Addis Ababa
//     zoom: 13
//   });
//
//   // Fetch real location playback history from the API on mount
//   React.useEffect(() => {
//     fetch('http://localhost:8099/api/devices/354017113649335/locations?limit=500')
//         .then((res) => {
//           if (!res.ok) throw new Error('Network response was not ok');
//           return res.json();
//         })
...
//             setApiLocations(data);
//
//             // Construct a valid active vehicle matching your backend identity
//             const activeVehicle: Vehicle = {
//               deviceId: "", type: undefined,
//               id: "354017113649335", // Critical: Use the matching tracker ID
//               name: "Active Tracker Truck",
//               plate: "ET-35401",
//               status: data[0].speed > 0 ? 'moving' : 'idle',
//               location: [data[0].latitude, data[0].longitude],
//               speed: Math.round(data[0].speed),
//               fuel: 85.5,    // Base fallback baseline metrics
//               battery: 98.0,
//               lastPing: data[0].timestamp,
//               currentHistoryIndex: 0
//             };
